...
let current_user = ["abdullah", "ali", "adeel", "hamza", "basit"];
let new_user = ["sherry", "moiz", "abdullah", "ABDULLAH", "adeel"];

for (let i = 0 ; i < new_user.length ; i++)
{
    if(current_user.includes(new_user[i]))
    {
        console.log(`SORRY, The username ${new_user[i]} should not acceted as it is already used `)
    }
    else
    {
